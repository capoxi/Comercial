...
class Form extends Base {
    ...
    public function __construct($id = "pfbc") {
        ...
        $path = $_SERVER["SERVER_NAME"] . ":" . $_SERVER["SERVER_PORT"] . "/PFBC/Resources";
    ...
    protected function setActiveProp($sum = "") {
        $prop = array();
        // rc - ler/criar (novo); ra - ler/alterar; 
        // ro - só leitura; rx - excluir
        if ($this->_mode == "ro" || "rx") {
            $prop = array("readonly" => 1, "disabled" => 1);
            array_push($this->_elProp, $prop);
        }
        array_push($this->_elProp, $sum);
    }

    public function setHeader($mode = "rc", $ativo = "S", $legend = "") {
        $this->_legend = $legend;
        $this->_mode = $mode;

        $this->setActiveProp();

        $this->addElement(new Element\HTML('<legend>' . $legend . '</legend>'));
        // Elemento ativo mostrará em situações diferentes de cadastro.
        if ($this->_mode == "rc") {
            $this->addElement(new Element\Hidden("ativo", "S")
            );
        }  else {
            $this->addElement(new Element\SimNao("Ativo", "ativo", array($this->_elProp,"required" => 1, "value" => $ativo)
            ));
        }
    }
    ...
    public function setFooter() {
        // rc - ler/criar (novo); ra - ler/alterar; ro - só leitura; rx - excluir
        switch ($this->_mode) {
            case "rc" :
                $this->defLabel = "Cadastrar";
                break;
            case "ra":
                $this->defLabel = "Salvar";
                break;
            case "ro":
                $this->defLabel = "-";
                break;
            case "rx" :
                $this->defLabel = "Excluir";
                break;
        }
        $this->addElement(new Element\Button($this->defLabel));
        $this->addElement(new Element\Button("Cancelar", "button", array(
    ...
    protected function renderCSSFiles() {
        ...
        if (!in_array("bootstrap", $this->prevent))
            $urls[] = $this->_prefix . "://localhost:8080/Comercial/formbuilder/bootstrap/css/bootstrap-combined.min.css";
...
JS;
    ...
    protected function renderJSFiles() {
        ...
        if (!in_array("jQuery", $this->prevent))
            $urls[] = $this->_prefix . "://localhost:8080/Comercial/formbuilder/jquery-ui-1.9.2.custom/js/jquery-1.8.3.js";
        if (!in_array("bootstrap", $this->prevent))
            $urls[] = $this->_prefix . "://localhost:8080/Comercial/formbuilder/bootstrap/js/bootstrap.min.js";
